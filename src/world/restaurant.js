...
/**
 * Tiles a three-tile-high piece paints over, relative to the piece.
 *
 * The pass counter and the host desk are the only fixtures that draw taller
 * than the tile behind them, and in painter's order a guest one row up-screen
 * sorts *earlier* — so the counter lands on top and cuts them off at the chest.
 * Rather than special-case the sort, the room treats those three tiles as the
 * staff side of the counter: guests neither queue there nor path through, and
 * nothing you can sit on may be built there.
 */
const BEHIND = [[-1, 0], [0, -1], [-1, -1]];
...
export class Restaurant {
    ...
    for (const f of this.grid.values()) {
      ...
      if (f.item.tall) {
        for (const [dc, dr] of BEHIND) {
          ...
          // the doorway is never staff side, or nobody could get in or out
          if (c === this.entry.c && r === this.entry.r) continue;
          this.behind.add(this.key(c, r));
        }
      }
    }
    for (const f of this.grid.values()) {
  ...
  /**
   * Solid tiles block movement; guests walk through each other happily. The
   * strip behind a tall counter is staff side and stays clear of guests.
   */
  walkable = (c, r) => this.room.inside(c, r)
    && !this.grid.has(this.key(c, r))
    && !this.behind.has(this.key(c, r));
